    ...
    <div class="tf-row-flex">
        <div class="wrapper-control-shop tf-shop-content">
            ...
            <div class="tf-list-layout wrapper-shop" id="listLayout" style="display: none;">
                ...
                <div v-else v-for="product in filteredProducts" :key="product.id" class="card-product list-layout" :data-availability="product.stok > 0 ? 'In stock' : 'Out of stock'" data-brand="Ecomus">
                    <div class="card-product-wrapper">
                        <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }" class="product-img">
                            <img v-if="product.image_urls && product.image_urls.length > 0" class="lazyload img-product" :data-src="product.image_urls[0]" :src="product.image_urls[0]" alt="image-product">
                            <!-- Add a placeholder image if no image is available -->
                        </router-link>
                    </div>
                    <div class="card-product-info">
                        <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }" class="title link">{{ product.nama_produk }}</router-link>
                        <span class="price current-price">{{ formatPrice(product.harga) }}</span>
                        <p class="description">{{ product.deskripsi }}</p>
                        <ul class="list-color-product">
                            <!-- Color swatches can be dynamic if color info is available in product data -->
                        </ul>
                        <div class="size-list">
                            <!-- Size list can be dynamic if size info is available in product data -->
                        </div>
                        <div class="list-product-btn">
                            <!-- Add to cart, wishlist, compare, quick view buttons -->
                            <a href="#" @click.prevent="addToCart(product)" class="box-icon quick-add style-3 hover-tooltip"><span class="icon icon-bag"></span><span class="tooltip">Quick add</span></a>
                            <a href="#" @click.prevent="addToWishlist(product)" class="box-icon wishlist style-3 hover-tooltip"><span class="icon icon-heart"></span> <span class="tooltip">Add to Wishlist</span></a>
                            <a href="#" @click.prevent="viewProductDetail(product.id)" class="box-icon quickview style-3 hover-tooltip"><span class="icon icon-view"></span><span class="tooltip">Quick view</span></a>
                        </div>
                    </div>
                </div>
                <!-- /card product -->
            ...
            <div class="wrapper-shop tf-grid-layout tf-col-3" id="gridLayout">
                ...
                <div v-else-if="errorMessage" class="error-message">{{ errorMessage }}</div>
                <div v-else-if="filteredProducts.length === 0" class="empty-message">No products found.</div>
                <div v-else v-for="product in filteredProducts" :key="product.id" class="card-product" :data-availability="product.stok > 0 ? 'In stock' : 'Out of stock'" data-brand="Ecomus">
                    <div class="card-product-wrapper">
                        <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }" class="product-img">
                            <img v-if="product.image_urls && product.image_urls.length > 0" class="lazyload img-product" :data-src="product.image_urls[0]" :src="product.image_urls[0]" alt="image-product">
                            <!-- Add a placeholder image if no image is available -->
                        ...
                        <div class="list-product-btn">
                            <a href="#" @click.prevent="addToCart(product)" class="box-icon bg_white quick-add tf-btn-loading">
                                <span class="icon icon-bag"></span>
                                <span class="tooltip">Quick add</span>
                            </a>
                            <a href="#" @click.prevent="addToWishlist(product)" class="box-icon bg_white wishlist btn-icon-action">
                                <span class="icon icon-heart"></span>
                                <span class="tooltip">Add to Wishlist</span>
                            </a>
                            <a href="#" @click.prevent="viewProductDetail(product.id)" class="box-icon bg_white quickview tf-btn-loading">
                    ...
                    <div class="card-product-info">
                        <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }" class="title link">{{ product.nama_produk }}</router-link>
                        <span class="price">{{ formatPrice(product.harga) }}</span>
                        <ul class="list-color-product">
                            <!-- Color swatches can be dynamic if color info is available in product data -->
                        </ul>
                    </div>
...
const addToCart = (product: any) => {
    console.log('Add to cart:', product.nama_produk);
    alert(`Added ${product.nama_produk} to cart (not implemented yet).`);
};

const addToWishlist = (product: any) => {
    console.log('Add to wishlist:', product.nama_produk);
    alert(`Added ${product.nama_produk} to wishlist (not implemented yet).`);
};
...
function setGridLayout(layoutClass: string) {
    ...
    $('#gridLayout')
        .show()
        .removeClass('tf-col-2 tf-col-3 tf-col-4')
        .addClass(`tf-grid-layout ${layoutClass}`);
