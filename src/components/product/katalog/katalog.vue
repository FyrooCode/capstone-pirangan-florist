            ...
            <div class="tf-list-layout wrapper-shop" id="listLayout" style="display: none;">
                ...
                <div v-else v-for="product in filteredProducts" :key="product.id" class="card-product list-layout"
                    ...
                    <div class="card-product-info">
                        <router-link :to="{ name: 'ProductDetail', params: { id: product.id } }" class="title link">{{
                    ...
                    <ul class="list-categoris current-scrollbar mb_36">
                        ...
                        <li v-for="category in categories" :key="category.id" class="cate-item"
                            ...
                            <a href="#" @click.prevent="selectCategory(category.id)">
                                <span>{{ category.nama_kategori }}</span>&nbsp;<span>({{ category.product_count
                                    }})</span>
                            </a>
                        </li>
                    </ul>
...
const addToCart = (product: any) => {
    // Open quick add modal instead of direct add to cart
...
const handleAddToCartFromModal = async (product: any, quantity: number) => {
    try {
        const success = await cartStore.addToCart(product, quantity);
        if (success) {
            console.log('Added to cart from modal:', product.nama_produk, 'Quantity:', quantity);
            // You can add a toast notification here
            alert(`Added ${quantity}x ${product.nama_produk} to cart!`);
        } else {
...
/* Prevent any scaling or transform effects */
.card-product:hover .img-product,
